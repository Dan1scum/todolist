...
def main(page: ft.Page):
    # print('Hello Flet')
    page_theme = ft.ThemeMode.LIGHT

    task_list = ft.Column(spacing=25)

    filter_type = 'all' # uncompleted, completed
    ...
    def view_tasks(task_id, task_text, completed=None):
        ...
        checkbox = ft.Checkbox(value=bool(completed), on_change=lambda e: toggle_task(checkbox=checkbox, task_id=task_id, is_completed=e.control.value))
    ...
    def add_task_db(_):
        if task_input.value:
            ...
            print(f'Adding {task} task: ID - {task_id}')
            task_list.controls.append(view_tasks(task_id=task_id, task=task))
            task_input.value = None
    
    task_input = ft.TextField(label='New Task', expand=True, on_submit=add_task_db)
